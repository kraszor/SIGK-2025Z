"""
Training script for SRCNN with Ray Tune
Uses MSE Loss
"""
...
from models.image_sr import SRCNN
...
def train_srcnn(config, data_dir: str, checkpoint_dir: str = None):
...
def main(data_dir: str, num_samples: int = 10, num_epochs: int = 50, input_size: int = 32):
    ...
    config = {
        "in_channels": 3,
        "out_channels": 3,
        "feat1": tune.choice([64, 96, 128]),
        "feat2": tune.choice([32, 48, 64]),
        "k1": tune.choice([7, 9, 11]),
        "k2": tune.choice([1, 3]),
    ...
    result = tune.run(
        tune.with_parameters(train_srcnn, data_dir=data_dir),
    ...
    final_model_path = "./best_srcnn_model.pth"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train SRCNN with Ray Tune")
