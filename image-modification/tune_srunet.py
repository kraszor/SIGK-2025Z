...
from models.image_sr import SRUNet
...
def setup_logging(init_features: int, slurm_job_id: str = None):
    ...
    if slurm_job_id:
        log_filename = f"srunet_training_init{init_features}_job{slurm_job_id}_{timestamp}.log"
    else:
        log_filename = f"srunet_training_init{init_features}_{timestamp}.log"
    ...
    logger.info("="*80)
    logger.info(f"Starting SR-U-Net training with init_features={init_features}")
    if slurm_job_id:
...
def train_model(data, init_features, input_size, epochs=10, patience=10):
    try:
        ...
        for epoch in range(epochs):
            ...
            if val_loss < best_val_loss:
                ...
                epochs_without_improvement = 0
                model_filename = f"best_model_SR-unet_init{init_features}"
                if slurm_job_id:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train SR-U-Net with comprehensive logging for Slurm")
    ...
    print("="*80)
    print("SR-U-Net Training Experiment")
    ...
    full_dataset = ImageDataset(root=data_dir, input_size=(args.input_size, args.input_size))
    ...
    best_val_loss, best_val_psnr = train_model((train_loader, val_loader), args.init_features, args.input_size, args.epochs, args.patience)
